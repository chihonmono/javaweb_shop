...
<%--商品展示--%>
<table border="1" class="table table-bordered table-hover table-outside">
    <c:if test="${empty pb.list}">
        <h3 style="margin-top: 80px">好像没找到该商品？请再次搜索</h3>
    </c:if>
    <tr>
    ...
    <tr>
        ...
        <td class="td-out">
            <a href="javascript:goDetail('${good.g_id}','${user.u_id}','')">
                <table class="table-inside" id="card">
                    <tr><td><img class="img" src="image/goods/${good.g_img}.jpg"/></td></tr>
                    <tr><td><h3>${good.g_name}</h3></td></tr>
                    <tr><td><h3>${good.g_price}</h3></td></tr>
                </table>
            </a>
        </td>
            </c:forEach>
...
<script>
    function goDetail(g_id,u_id,gd_msg) {
        if (u_id == ""){
            u_id = "0";
        }
        location.href="${pageContext.request.contextPath}/goodDetailServlet?g_id="+g_id+"&u_id="+u_id+"&gd_msg";
    }
</script>
</body>
</html>